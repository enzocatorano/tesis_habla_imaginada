# src/resultados_TL.py
"""
Script de visualización y resumen de resultados para experimentos TRANSFER LEARNING.

Genera dos tipos de análisis:
1. POR SUJETO HELD-OUT: Learning curves, Matrices de confusión, Boxplots P/R/F1.
...
EXPERIMENT_NAME_PREFIX = "TL_DeepConvNet_clean_20260525-054605_CatoranoBrothers"
...
def collect_tl_data(heldout_dir: Path, subset: str):
    """
    Recolecta datos de un sujeto held-out para transfer learning.
    La estructura esperada es:
      <heldout_dir>/<subset>/seed_X/metadata.json
      <heldout_dir>/<subset>/seed_X/classification_report.json
      <heldout_dir>/<subset>/seed_X/confusion_matrix.npy
      <heldout_dir>/<subset>/seed_X/train_metrics.json
    """
    subset_dir = heldout_dir / subset
    ...
    for seed_dir in seed_dirs:
        meta = load_json_safe(seed_dir / "metadata.json")
        ...
        cr = load_json_safe(seed_dir / "classification_report.json")
        if cr:
        ...
        cm_path = seed_dir / "confusion_matrix.npy"
        if cm_path.exists():
            try:
                cm = np.load(cm_path)
        ...
        train_metrics = load_json_safe(seed_dir / "train_metrics.json")
        if train_metrics:
...
def plot_metrics_boxplots(subject_data_dict, output_path: Path, subject_name: str):
    ...
    for subset_name, data in subject_data_dict.items():
        ...
        n_classes = data.get("n_classes") or 11
...
def plot_global_metrics_boxplots(all_subjects_data: Dict, output_path: Path, subsets: List[str]):
    ...
    for subset_name in subsets:
        ...
        for subj_name, subj_data in all_subjects_data.items():
            ...
            if s_sum and s_sum.get("n_classes"):
                n_classes = s_sum["n_classes"]
                break
        subset_n_classes[subset_name] = n_classes or 11
    ...
    for subset_name in subsets:
        ...
        n_classes = subset_n_classes[subset_name]
        chance = 1.0 / n_classes
...
def plot_global_precision_by_subject(all_subjects_data: Dict, output_path: Path, subsets: List[str]):
    ...
    for ax, subset_name in zip(axes, subsets):
        ...
        for subj_name in subject_names:
            ...
            if subj_data:
                subset_data = subj_data.get(subset_name)
                if subset_data and subset_data.get("n_classes"):
                    n_classes = subset_data["n_classes"]
                    break
        if n_classes is None:
            n_classes = 11
        chance = 1.0 / n_classes
...
def process_heldout(heldout_dir: Path, output_root: Path, subsets: List[str]):
    """
    Procesa un sujeto held-out (directorio con nombre <Sxx>_heldout).
    La estructura esperada es:
      <heldout_dir>/<subset>/seed_X/...
    """
    subject_name = heldout_dir.name
    print(f"\n[Visualizer TL] Processing: {subject_name}")
    ...
    for subset in subsets:
        data = collect_tl_data(heldout_dir, subset)
        if data:
    ...
    subject_data["summary"] = summary
    return subject_data

# ==================== MAIN ====================
if __name__ == "__main__":
    EXP_ROOT = find_latest_experiment(EXPERIMENTS_ROOT, EXPERIMENT_NAME_PREFIX)
    print(f"[Visualizer TL] Using experiment root: {EXP_ROOT}")
    ...
    print(f"[Visualizer TL] Expected subsets: {expected_subsets}")
    ...
    heldout_dirs = sorted([
        ...
        if p.is_dir() and "_heldout" in p.name
    ])

    all_subjects_data = {}
    for heldout_dir in heldout_dirs:
        data = process_heldout(heldout_dir, OUTPUT_ROOT, expected_subsets)
        if data is not None:
            all_subjects_data[heldout_dir.name] = data

    print("\n[Visualizer TL] Generating global visualizations...")
    ...
    print(f"\n[Visualizer TL] Done! Results saved to: {OUTPUT_ROOT}")
